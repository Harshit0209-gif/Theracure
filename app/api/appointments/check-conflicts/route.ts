...
import { conflictCheckSchema, ConflictCheckRequest } from "@/lib/validations/appointment"
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!data) {
      return NextResponse.json({ error: 'Invalid validation data' }, { status: 400 })
    }

    const { therapistId, appointmentStartTime, appointmentEndTime, excludeAppointmentId } = data
    ...
    const conflicts = await prisma.therapistAssignment.findMany({
      where: {
        therapistId,
        status: { in: ['confirmed'] },
    ...
    const formattedConflicts = conflicts.map(conflict => ({
      ...
      therapyType: conflict.therapyType,
      startTime: conflict.appointmentStartTime.toISOString(),
      endTime: conflict.appointmentEndTime.toISOString(),
      duration: Math.round(
        (conflict.appointmentEndTime.getTime() - conflict.appointmentStartTime.getTime()) / (1000 * 60)
      )
    }))
