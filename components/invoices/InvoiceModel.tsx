...
function InfoRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2">
      <span className="text-xs text-gray-400 font-medium uppercase tracking-wide whitespace-nowrap">{label}</span>
...
export const InvoiceDetailsModal = ({
  ...
  if (!isValid) {
    toast({ title: "Missing Data", description: "Some invoice data is incomplete.", variant: "destructive" });
    return null;
  }
        ...
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto p-0 gap-0">
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-mono text-gray-400 mt-0.5">{invoiceDetails.id}</p>
                </div>
              </div>
              <Badge
          ...
          <div className="px-6 py-5 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <SectionHeading icon={<User className="h-4 w-4" />} title="Patient" />
              ...
              {/* Invoice meta */}
              <div className="bg-gray-50 rounded-xl p-4">
                <SectionHeading icon={<FileText className="h-4 w-4" />} title="Invoice Info" />
                <div className="divide-y divide-gray-100">
                  <InfoRow
                    label="Date"
                    value={
            ...
            {/* ── Services table ── */}
            <div>
              <SectionHeading icon={<Activity className="h-4 w-4" />} title="Services & Items" />
              <div className="rounded-xl overflow-hidden border border-gray-100">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-indigo-700 text-white">
                      <th className="text-left px-4 py-2.5 font-semibold text-xs">Service / Item</th>
                      <th className="text-center px-4 py-2.5 font-semibold text-xs">Sessions</th>
                      <th className="text-right px-4 py-2.5 font-semibold text-xs">Rate</th>
                      <th className="text-right px-4 py-2.5 font-semibold text-xs">Amount</th>
                    </tr>
                  </thead>
                    ...
                    {invoiceDetails.invoiceItems?.length ? (
                      invoiceDetails.invoiceItems.map((item: any, index: number) => (
                        <tr key={index} className="hover:bg-indigo-50/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right text-gray-600">₹{item.priceAtPurchase.toFixed(2)}</td>
            ...
            {invoiceDetails.transactions?.length > 0 && (
              <div>
                <SectionHeading icon={<CreditCard className="h-4 w-4" />} title="Transaction History" />
                <div className="rounded-xl overflow-hidden border border-gray-100">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-indigo-700 text-white">
                        <th className="text-left px-4 py-2.5 font-semibold text-xs">Transaction ID</th>
                        <th className="text-left px-4 py-2.5 font-semibold text-xs">Date & Time</th>
                        <th className="text-right px-4 py-2.5 font-semibold text-xs">Amount</th>
                        <th className="text-left px-4 py-2.5 font-semibold text-xs">Mode</th>
                        <th className="text-center px-4 py-2.5 font-semibold text-xs">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50 bg-white">
                      {invoiceDetails.transactions.map((txn: any) => (
                        <tr key={txn.id} className="hover:bg-indigo-50/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-gray-600 capitalize text-xs">{txn.paymentMethod || "N/A"}</td>
            ...
            {/* ── Payment summary ── */}
            <div className="bg-gray-50 rounded-xl p-4">
              <SectionHeading icon={<IndianRupee className="h-4 w-4" />} title="Payment Summary" />
          ...
          <DialogFooter className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-end gap-2">
            ...
            {status === InvoiceStatus.DUE && (
              <Button onClick={() => setIsPaymentDialogOpen(true)} className="bg-emerald-600 hover:bg-emerald-700">
